...
def train(args,model, train_loader, optimizer, loss_fun, global_protos, client_num, device,  server_model, train_epoch=-1):
    ...
    for step in range(len(train_iter)):
        ...
        if train_epoch != 0:
            ...
            # server_model第三次输出
            optimizer.zero_grad()
            server_model_middle_output3, _ = server_model(x,args.output_layer3)  # TODO 应该把这个也设置成server_model的输入
            input_layer3 = args.output_layer3 + 1
            middleoutput3, middle_protos3 = model(server_model_middle_output3, -1, input_layer3)

            middle_loss3_1 = loss_fun(middleoutput3, y)

            middle_loss3_2 = count_loss2(middle_loss3_1, middle_protos3, label_g, global_protos)

            middle_loss3 = middle_loss3_1 + middle_loss3_2 * args.ld
            # Backward pass
            middle_loss3.backward()
            loss_all += middle_loss3.item()
            optimizer.step()
            # Compute accuracy
            pred = middleoutput3.data.max(1)[1]
            correct += pred.eq(y.view(-1)).sum().item()

            for i in range(len(y)):
                if label_g[i].item() in agg_protos_label:
                    agg_protos_label[label_g[i].item()].append(middle_protos3[i, :])
                else:
                    agg_protos_label[label_g[i].item()] = [middle_protos3[i, :]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_layer3', type=int, default=2, help='output layer for forward')
