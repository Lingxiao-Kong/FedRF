...
import time
import copy
from nets.models_old import DigitModel, ResNet18, BasicBlock
import argparse
import numpy as np
import torchvision
...
if __name__ == '__main__':
    ...
    # 这里使用的是fedbn中的模型进行模型的初始化
    server_model = ResNet18(BasicBlock, [2, 2, 2, 2], 1, 10)
    ...
    for a_iter in range(resume_iter, args.iters):
        ...
        # report after aggregation 聚合后的client输出
        for client_idx in range(client_num):
            model, train_loader, optimizer = models[client_idx], train_loaders[client_idx], optimizers[client_idx]
            train_loss, train_acc = test(model, train_loader, loss_fun, device)
