...
class Client:
    def __init__(self, criterion, trainset, testset, client_config, cid, device, **kwargs):
        ...
        if trainset is not None:
            self.num_train_samples = len(trainset.dataset)
        else:
            self.num_train_samples = 0
        if testset is not None:
            self.num_test_samples = len(testset.dataset)
        else:
    ...
    def _prepare_data(self):
        ...
        if self.num_train_samples > 0:
            self.trainloader = self.trainset
            ...
            self.count_by_class = Counter(self.trainset.dataset.labels)
        ...
        if self.num_test_samples > 0:
            self.testloader = self.testset
            # self.testloader = DataLoader(self.testset, batch_size=test_batchsize, shuffle=False)
            self.count_by_class_test = Counter(self.testset.dataset.labels)
