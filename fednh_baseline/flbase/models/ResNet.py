...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_feature=False):#32*3*28*28
        out = F.relu(self.bn1(self.conv1(x)))#32*64*28*28
        out = self.layer1(out)
        out = self.layer2(out)#32*128*14*14
        self.activation = out
        out = self.layer3(out)#32*256*7*7
        out = self.layer4(out)#32*512*4*4
        # self.activation = out
        out = F.adaptive_avg_pool2d(out, 1)#32*512*1*1
