...
class FedAvgClient(Client):
    ...
    def training(self, round, num_epochs):
        ...
        for i in range(num_epochs):
            ...
            for j, (x, y) in enumerate(self.trainloader):
                ...
                loss = self.criterion(yhat, y.long())
                # backward pass
    ...
    def testing(self, round, testloader=None):
        ...
        test_count_per_class = Counter(self.trainset.dataset.labels)
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(testloader):
                ...
                yhat = self.model.forward(x)
                # loss
                test_loss += self.criterion(yhat, y.long()).item()
                # stats
                predicted = yhat.data.max(1)[1]
        ...
        correct_num = test_correct_per_class.sum().item()
...
class FedAvgServer(Server):
    def __init__(self, server_config, clients_dict, exclude, **kwargs):
        ...
        for cid in range(self.num_clients):
            # same initial weight
            self.server_side_clients[cid].set_params(self.server_model_state_dict, exclude_keys=set())
    ...
    def testing(self, round, active_only=True, **kwargs):
        ...
        # get five global client
        test_loss = 0
        test_correct = 0
        for cid in range(self.num_clients):
            test_loaders_len = len(self.server_side_clients[cid].trainloader)
            test_loaders_dataset_len = len(self.server_side_clients[cid].trainloader.dataset)
            self.server_side_clients[cid].set_params(self.server_model_state_dict, self.exclude_layer_keys)
            client_loss,client_correct = self.server_side_clients[cid].testing(round, testloader=self.server_side_clients[cid].trainloader)  # use global testdataset
            train_loss = client_loss / test_loaders_len
            train_correct = client_correct / test_loaders_dataset_len
            print(
                '聚合后client： {:<11d}| Train Loss: {:.4f} | Train Acc: {:.4f}'.format(cid, train_loss,
                                                                                       train_correct))
        for cid in range(self.num_clients):
            test_loaders_len = len(self.server_side_clients[cid].testloader)
            test_loaders_dataset_len = len(self.server_side_clients[cid].testloader.dataset)
            self.server_side_clients[cid].set_params(self.server_model_state_dict, self.exclude_layer_keys)
            client_loss,client_correct = self.server_side_clients[cid].testing(round, testloader=None)  # use global testdataset
            test_loss = client_loss / test_loaders_len
            test_correct = client_correct / test_loaders_dataset_len
            print(
                '聚合后client： {:<11d}| Test Loss: {:.4f} | Test Acc: {:.4f}'.format(cid, test_loss,
                                                                                       test_correct))
