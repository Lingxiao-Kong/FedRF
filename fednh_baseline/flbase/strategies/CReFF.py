...
class CReFFClient(Client):
    ...
    def compute_real_feature_gradient(self, global_retrained_model_state_dict):
        ...
        images_all = [torch.unsqueeze(self.trainset.dataset[i][0], dim=0) for i in range(len(self.trainset.dataset))]
        try:
            labels_all = [self.trainset.dataset[i][1].item() for i in range(len(self.trainset.dataset))]
        except AttributeError:
            labels_all = [self.trainset.dataset[i][1] for i in range(len(self.trainset.dataset))]
