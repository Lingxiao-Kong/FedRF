    ...
    def _train_together(self, round, num_epochs):
        ...
        for i in range(num_epochs):
            ...
            for j, (x, y) in enumerate(self.trainloader):
                ...
                loss_bsm = balanced_softmax_loss(y.long(), out_g, self.sample_per_class)
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(testloader):
                ...
                if self.is_on_server:
                    ...
                    out = out_p + out_g
                # loss
                test_loss += self.criterion(out, y.long()).item()
                # stats
                predicted = out.data.max(1)[1]
...
def balanced_softmax_loss(labels, logits, sample_per_class, reduction="mean"):
    ...
    loss = F.cross_entropy(input=logits, target=labels.long(), reduction=reduction)
    return loss
