...
def train(args,model, train_loader, optimizer, loss_fun, global_protos , client_num, device , output_layer1 ,server_model, train_epoch = -1 ):
    ...
    for step in range(len(train_iter)):
        ...
        # Zero the gradients
            optimizer.zero_grad()
            server_model_middle_output,_  = server_model(x, output_layer1)  # 这部分的proto是server,这里用不到middle_proto
            input_layer = output_layer1 + 1
            middleoutput ,middle_protos = model(server_model_middle_output, -1 ,input_layer,True) #client model ,TODO 用这里的proto计算loss2

            middle_loss1 = loss_fun(middleoutput , y)

            middle_loss2 = count_loss2(middle_loss1,middle_protos,y,global_protos)

            middle_loss = middle_loss1 + middle_loss2 * args.ld
            ...
            for i in range(len(y)):
                if label_g[i].item() in agg_protos_label:
                    agg_protos_label[label_g[i].item()].append(middle_protos[i, :])
                else:
                    agg_protos_label[label_g[i].item()] = [middle_protos[i, :]]


    # Return average loss and accuracy
    return loss_all/len(train_iter)*2, correct/num_data*2 , agg_protos_label
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wk_iters', type = int, default=2, help = 'optimization iters in local worker between communication')
    ...
    proto_loss_fun = nn.NLLLoss().to(device)
            ...
            for client_idx in range(client_num):
                ...
                if args.mode.lower() == 'fedprox':
                    if a_iter > 0:
                        ...
                        print('聚合前client本地更新结果:  {:<11s}| Train Loss: {:.4f} | Train Acc: {:.4f}'.format(datasets[client_idx] ,train_loss, train_acc))
                        test_loss, test_acc = test(models[client_idx], test_loader, loss_fun, device)
                        print('聚合前client本地更新结果:  {:<11s}| Test Loss: {:.4f} | Test Acc: {:.4f}'.format(datasets[client_idx] ,test_loss, test_acc))

                else:
                    loss_all, correct , protos=train(args,model, train_loader, optimizer, loss_fun, global_protos , client_num, device ,args.output_layer , server_model, a_iter )
