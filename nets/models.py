...
class ResNet18(nn.Module):
    ...
    def forward(self, x, output_layer=-1, input_layer=-1, output_proto=False):  # 32,3,28,28 TODO 这里需要改一下，在第四层输出一个
        ...
        if input_layer <= 1:
            ...
            layer1_result = x;
            if output_layer == 1 and output_proto == False:
                return x , None
        ...
        if input_layer <= 2:
            ...
            x = self.layer1(x)
            layer2_result = x;
            if output_layer == 2 and output_proto == False:
                return x , None
        if input_layer <= 3:
            x = self.layer2(x)  # 32,128,4,4
            layer3_result = x;
            if output_layer == 3 and output_proto == False:
                return x , None
        if input_layer <= 4:
            x = self.layer3(x)  # 32,256,2,2
            layer4_result = x
            if output_layer == 4 and output_proto == False:
                return x ,None
        if input_layer <= 5:
            x = self.layer4(x)  # 32,512,1,1
            proto = x
            if output_layer == 5 and output_proto == False:
                return x , None
        if output_proto == True:
            if output_layer == 1:
                return layer1_result , proto
            if output_layer == 2:
                return layer2_result , proto
            if output_layer == 3:
                return layer3_result , proto
            if output_layer == 4:
                return layer4_result , proto
            if output_layer == 5:
                return proto , proto
        if input_layer == 6:
            proto = x
